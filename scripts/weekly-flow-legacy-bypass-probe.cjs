...
for(const file of [
  ...
  'docs/patch-proposals/20260920_exam_prep_weekday_availability_optional_v1.sql',
  'supabase/tests/exam_prep_weekday_availability_isolated_matrix.sql'
]) {
  const check=spawnSync('psql',['-X','-v','ON_ERROR_STOP=1','-f',file],{
    env,encoding:'utf8',timeout:60000
  });
  if(check.stdout) process.stdout.write(check.stdout.slice(-2500));
  assert.equal(check.status,0,`Isolated proposal test failed in ${file}: ${(check.stderr||'').slice(-3500)}`);
}
console.log('PRIOR WEEK + OPTIONAL WEEKDAY HOURS isolated SQL GREEN; no academic writes or surviving synthetic student.');
