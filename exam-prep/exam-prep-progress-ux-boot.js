...
(() => {
  ...
  const fail = () => { internal.progressUxBootstrapStatus = 'unavailable'; };
  ...
  function loadScript(name, datasetKey, expected) {
    return new Promise((resolve, reject) => {
      ...
      el.src = `${base}exam-prep-progress-ux-${name}.js?v=progressux1`;
      el.async = false;
  ...
  async function boot() {
    try {
      ...
      if (!document.querySelector('link[data-exam-prep-progress-ux-style]')) {
        ...
        link.href = `${base}exam-prep-progress-ux.css?v=progressux1`;
