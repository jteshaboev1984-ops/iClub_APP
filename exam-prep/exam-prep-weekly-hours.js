/* Optional weekly Mathematics hours, never a second plan editor.
 * Loaded only behind the owner-controlled weekly-flow switch. The server must
 * independently authorize each call. No storage, auto-replan or grade mutation.
 */
(() => {
  'use strict';
  ...
  const words = {
    ru: {
      ...
      invalid:'Заполните все семь дней или оставьте все пустыми. Допустимы значения от 0 до 24 с шагом 0,5 часа. Сумма не должна превышать недельный бюджет математики.',
  ...
  function allowed() {
    ...
    return window.iClubExamPrepWeeklyFlowEnabled === true &&
      caps?.coreAccess === true && caps?.killSwitch === false && caps?.rolloutState === 'controlled_beta';
  }
  function copy() {
    let lang='ru';
    try { lang=String(window.i18n?.getLang?.() || document.documentElement.lang || 'ru').toLowerCase(); }
    catch (_) { /* use Russian fallback */ }
  ...
  function validSlots(value) {
    ...
    return value && typeof value==='object' && !Array.isArray(value) &&
      Object.keys(value).length===7 && DAYS.every(day=>
        Object.hasOwn(value,day) && typeof value[day]==='number' &&
        Number.isFinite(value[day]) && value[day]>=0 && value[day]<=24 &&
        Number.isInteger(value[day]*2));
  }
  function validRead(data,profile) {
    if (data?.contract_version!==VERSION || data.enabled!==true ||
        data.scope!=='shared_mathematics_week' ||
        data.planning_only!==true || data.does_not_change_current_plan!==true ||
        typeof data.confirmed!=='boolean' || typeof data.needs_reconfirmation!=='boolean' ||
        !Number.isInteger(data.profile_revision) || data.profile_revision<1 ||
        !Number.isFinite(Number(data.mathematics_hours_budget)) ||
        Number(data.mathematics_hours_budget)!==Number(profile.mathematics_hours_budget) ||
        !validSlots(data.weekday_hours) ||
        (data.weekday_hours===null && (data.confirmed || data.needs_reconfirmation)) ||
        (data.confirmed && data.needs_reconfirmation)) return false;
    return true;
  }
  async function decorate(form,profile) {
    ...
    if (!form.isConnected || !allowed() || !fetched.ok || !validRead(fetched.data,profile)) {
      form.dataset.epWeekdayHours='unavailable';
      return false;
    }
    const c=copy();
    const fields=document.createElement('fieldset');
    fields.className='ep-day-hours';
  ...
  async function save(form,profile) {
    ...
    const result=await rpc('save_my_exam_prep_profile_with_weekday_availability_safe_v1',{
      ...
      p_total_student_hours_available:profile.totalHours,
      p_mathematics_hours_budget:profile.mathHours,
      p_weekday_hours:available.days,
      p_expected_profile_revision:revision
    });
  ...
  internal.weeklyDayHours=Object.freeze({version:VERSION,decorate,read,save,invalidMessage:()=>copy().invalid});
})();
