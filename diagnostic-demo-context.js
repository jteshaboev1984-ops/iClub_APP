...
function makeContext(){
 const row=document.createElement('div');row.className='demo-student-context';row.dataset.profileId=D.profile.id;
 const avatar=document.createElement('span');avatar.className='demo-student-context-avatar';avatar.textContent=D.profile.initials||'СК';
 const copy=document.createElement('span');copy.className='demo-student-context-copy';const name=document.createElement('b');name.textContent=loc(D.profile.name);const status=document.createElement('small');status.textContent=loc(D.profile.status);copy.append(name,status);row.append(avatar,copy);return row;
}
function render(){
 document.documentElement.dataset.demoProfile=D.profile.id;
 const chatCopy=document.querySelector('.demo-ai-chat-title-copy');if(chatCopy){let row=chatCopy.querySelector('.demo-student-context');if(!row){row=makeContext();chatCopy.appendChild(row)}else{row.querySelector('b').textContent=loc(D.profile.name);row.querySelector('small').textContent=loc(D.profile.status)}}
 const trajectory=$('courses-pro-trajectory');if(trajectory&&!trajectory.querySelector('.demo-student-context')){const head=trajectory.querySelector('.demo-trajectory-head');if(head)head.insertAdjacentElement('afterend',makeContext())}
}
function patchTechnical(){
 setTimeout(()=>{
  ...
  const guardRow=rows.find(row=>/сетевая защита|tarmoq himoyasi|network guard/i.test(row.querySelector('span')?.textContent||''));
  if(guardRow)guardRow.querySelector('b').textContent='same-origin endpoint • production DB 0';
  if(!panel.querySelector('[data-profile-tech]')){
   const row=document.createElement('div');row.className='demo-technical-row';row.dataset.profileTech='1';const label=document.createElement('span');label.textContent=lang()==='ru'?'Профиль':lang()==='uz'?'Profil':'Profile';const value=document.createElement('b');value.textContent=`${D.profile.id} · ${loc(D.profile.status)}`;row.append(label,value);panel.appendChild(row)
  }
 },110)
}
