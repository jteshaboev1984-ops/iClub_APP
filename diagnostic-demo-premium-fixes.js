...
const COPY={
 ru:{current:'Текущий запрос',fromHistory:'Запрос из истории',all:'Все диалоги',latest:'Последний',dialogs:n=>`${n} ${n===1?'диалог':n>1&&n<5?'диалога':'диалогов'}`,firstPoint:'Первая точка',same:'Без изменений',attempt:n=>`П${n}`,skill:'Навык',why:'Почему система так считает',check:n=>`${n} ${n===1?'проверка':n>1&&n<5?'проверки':'проверок'}`},
 uz:{current:'Joriy savol',fromHistory:'Tarixdagi savol',all:'Barcha dialoglar',latest:'Oxirgi',dialogs:n=>`${n} ta dialog`,firstPoint:'Birinchi nuqta',same:'O‘zgarish yo‘q',attempt:n=>`U${n}`,skill:'Ko‘nikma',why:'Tizim nega shunday xulosa qildi',check:n=>`${n} ta tekshiruv`},
 en:{current:'Current question',fromHistory:'From history',all:'All conversations',latest:'Latest',dialogs:n=>`${n} ${n===1?'conversation':'conversations'}`,firstPoint:'First data point',same:'No change',attempt:n=>`A${n}`,skill:'Skill',why:'Why the system reached this conclusion',check:n=>`${n} ${n===1?'check':'checks'}`}
};
...
function updateContextBar(body,turn,isHistory,first){
 let bar=body.querySelector('[data-premium-dialog-context]');
 if(!bar){
  bar=node('div','premium-dialog-context');bar.dataset.premiumDialogContext='1';
  const copy=node('span','premium-dialog-context-copy');copy.append(node('b'),node('small'));
  const button=node('button','premium-dialog-history-link');button.type='button';button.dataset.premiumOpenHistory='1';
  bar.append(copy,button);
 }
 bar.querySelector('b').textContent=isHistory?t().fromHistory:t().current;
 bar.querySelector('small').textContent=dateText(turn?.user?.createdAt);
 bar.querySelector('button').textContent=t().all;
 if(first&&bar.nextElementSibling!==first)body.insertBefore(bar,first);
}
function renderCustomHistory(){
 const body=$('demo-ai-chat-body');if(!body)return;
 customHistoryOpen=true;
 if(body.dataset.premiumCustomHistory==='1')return;
 body.dataset.premiumCustomHistory='1';
 ...
 if(rows.length)body.appendChild(list);
...
function decorateHistory(){
 ...
 rows.forEach((row,index)=>{
  ...
  let badge=row.querySelector('.premium-history-latest');
  if(index===0&&!badge){badge=node('span','premium-history-latest');const arrow=row.querySelector('.settings-nav-arrow');arrow?row.insertBefore(badge,arrow):row.appendChild(badge)}
  if(badge)badge.textContent=t().latest;
...
function decorateDialog(){
 ...
 if(body.dataset.premiumCustomHistory==='1')delete body.dataset.premiumCustomHistory;
 if(body.querySelector('.demo-ai-history-head')){decorateHistory();return}
 const list=turns();
 if(!list.length){body.querySelector('[data-premium-dialog-context]')?.remove();return}
 applyingDialog=true;
 try{
  ...
  updateContextBar(body,selected,selected.id!==list[list.length-1].id,first);
...
function renderTrend(){
 ...
 const all=Array.isArray(history.diagnostics)?history.diagnostics:[];
 const rows=all.slice(-4);const base=Math.max(0,all.length-rows.length);
 ...
 rows.forEach((item,index)=>{
  ...
  track.appendChild(fill);wrap.append(value,track,node('span','premium-trend-label',t().attempt(base+index+1)));bars.appendChild(wrap);
 });
...
function pluralChecks(text){const match=String(text||'').match(/\d+/);return match?t().check(Number(match[0])):text}
function decorateSheet(){
 clearTimeout(sheetTimer);sheetTimer=setTimeout(()=>{
  ...
  const isSkill=Boolean(statusRow);sheet.classList.toggle('is-skill-detail',isSkill);
  let eyebrow=sheet.querySelector('.premium-sheet-eyebrow');let badge=sheet.querySelector('.premium-sheet-status');
  if(isSkill&&headCopy){
   if(!eyebrow){eyebrow=node('div','premium-sheet-eyebrow');headCopy.insertBefore(eyebrow,headCopy.firstChild)}eyebrow.textContent=t().skill;
   const source=statusRow.querySelector('b');
   if(source){if(!badge){badge=node('span','premium-sheet-status');headCopy.appendChild(badge)}badge.className=`premium-sheet-status ${source.className||''}`;badge.textContent=source.textContent}
  ...
  }else{eyebrow?.remove();badge?.remove()}
 },20)
}

function audit(){
 const tech=read(sessionStorage,TECH_KEY,{});
 const freeHidden=plan()!=='free'||$('demo-plan-result')?.classList.contains('premium-hide-free-result');
 const proBadge=Boolean(q('.demo-engine-compact .demo-plan-result-badge'));
 const button=q('.demo-engine-compact .demo-open-trajectory');
 const buttonFull=!button||Math.abs(button.getBoundingClientRect().width-(button.parentElement?.getBoundingClientRect().width||0))<=3;
 ...
 const report={version:'premium-fixes-v1.1',checkedAt:new Date().toISOString(),freeResultCardRemoved:freeHidden,duplicateProBadge:proBadge,trajectoryButtonFullWidth:buttonFull,trendVisible,overflowPx:overflow,pass:freeHidden&&!proBadge&&buttonFull&&trendVisible&&overflow<=2};
 write(sessionStorage,TECH_KEY,{...tech,premium_fixes_audit:report});return report;
}
function decorateAll(){renderTrend();decorateResult();scheduleDialog();decorateSheet();setTimeout(audit,140)}
...
document.addEventListener('click',event=>{
 ...
 const custom=event.target.closest('[data-premium-history-turn]');if(custom){selectedTurnId=custom.dataset.premiumHistoryTurn;customHistoryOpen=false;const body=$('demo-ai-chat-body');if(body)delete body.dataset.premiumCustomHistory;const tab=q('[data-chat-tab="dialog"]');tab?.click();setTimeout(scheduleDialog,50);return}
 ...
 const tab=event.target.closest('[data-chat-tab]');if(tab){customHistoryOpen=false;const body=$('demo-ai-chat-body');if(body)delete body.dataset.premiumCustomHistory;if(tab.dataset.chatTab==='dialog')selectedTurnId=null;setTimeout(scheduleDialog,60)}
...
const sheetRoot=$('demo-insight-sheet-root');if(sheetRoot)new MutationObserver(()=>decorateSheet()).observe(sheetRoot,{attributes:true,attributeFilter:['aria-hidden']});
