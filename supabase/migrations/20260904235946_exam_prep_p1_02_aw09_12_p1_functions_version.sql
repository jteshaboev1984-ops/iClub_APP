-- P1-02 AW9-12 P1 Functions content version. Draft/private only.
-- Covers composition/domain -> inverse/domain -> inverse graph reflection.
...
select pv.id,'p1_aw09_12_functions_v1','P1','P1 Functions FUN-03/04/05 AW9-12 governed runway','draft',
...
do $$ declare v_cfg private.exam_prep_feature_config%rowtype; v_active int; begin
  select * into v_cfg from private.exam_prep_feature_config where id=1;
  if v_cfg.rollout_state<>'off' or v_cfg.core_enabled or v_cfg.ai_enabled or v_cfg.mentor_enabled or not v_cfg.kill_switch then raise exception 'P1-02 AW9-12 functions version must remain fail-closed'; end if;
  select count(*) into v_active from private.exam_prep_feature_entitlements where entitlement_status='active';
  if v_active<>0 then raise exception 'P1-02 AW9-12 functions version found active entitlements=%',v_active; end if;
end $$;
commit;