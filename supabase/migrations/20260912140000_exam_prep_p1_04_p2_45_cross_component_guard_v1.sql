...
-- P2-45 / P1-04 hardening: AI Assist must not be recoverable by changing
-- client-supplied P1/P5 component while any protected Mathematics assessment
-- is active. P1/P5 mastery remains separate; this is only a subject-level
-- assistance blackout during protected assessment activity.
...
$$;
revoke all on function private.exam_prep_has_active_protected_assessment_v1(uuid) from public,anon,authenticated;
grant execute on function private.exam_prep_has_active_protected_assessment_v1(uuid) to service_role;
...
begin
  ...
  -- Deliberately independent from p_component_code. A client must not recover
  -- P1 AI while P5 is protected (or vice versa) by changing request metadata.
