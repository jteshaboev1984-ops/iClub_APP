-- P1-02 QA + publication gate for P1-QUA-04/05/06 AW9-12 pack.
-- Publishes governed Exam Prep memberships only. public.questions remain draft + inactive.
...
declare v_id bigint; v_skill text; v_bad int; begin
  ...
  if v_id is null then raise exception 'P1-02 AW9-12 quadratics QA: draft content version missing'; end if;
  if (select count(*) from private.exam_prep_question_content_meta where content_version_id=v_id)<>21 then
    raise exception 'P1-02 AW9-12 quadratics QA: expected 21 question objects';
  end if;

  foreach v_skill in array array['P1-QUA-04','P1-QUA-05','P1-QUA-06'] loop
    if (select count(*) from private.exam_prep_question_content_meta where content_version_id=v_id and primary_skill_code=v_skill and reserve_role='diagnostic')<>1 then raise exception 'P1-02 AW9-12 quadratics QA: % diagnostic floor',v_skill; end if;
    if (select count(*) from private.exam_prep_question_content_meta where content_version_id=v_id and primary_skill_code=v_skill and reserve_role='learning')<>3 then raise exception 'P1-02 AW9-12 quadratics QA: % learning floor',v_skill; end if;
    if (select count(*) from private.exam_prep_question_content_meta where content_version_id=v_id and primary_skill_code=v_skill and reserve_role='retest')<>2 then raise exception 'P1-02 AW9-12 quadratics QA: % retest floor',v_skill; end if;
    if (select count(*) from private.exam_prep_question_content_meta where content_version_id=v_id and primary_skill_code=v_skill and reserve_role='mixed')<>1 then raise exception 'P1-02 AW9-12 quadratics QA: % mixed floor',v_skill; end if;
    if (select count(*) from private.exam_prep_written_tasks where content_version_id=v_id and primary_skill_code=v_skill and lifecycle_state='approved')<>1 then raise exception 'P1-02 AW9-12 quadratics QA: % written floor',v_skill; end if;
  end loop;
  ...
  if v_bad<>0 then raise exception 'P1-02 AW9-12 quadratics QA: % payload/isolation/snapshot failures',v_bad; end if;
  ...
  if v_bad<>0 then raise exception 'P1-02 AW9-12 quadratics QA: diagnostic rule contract failed for % items',v_bad; end if;

  -- Reserve assessment items are holdouts; R02 remains completely unassigned.
  select count(*) into v_bad
  from private.exam_prep_assessment_items i
  join private.exam_prep_assessments a on a.id=i.assessment_id
  where a.content_version_id=v_id and a.assessment_type in ('diagnostic','retest','mixed') and not i.is_holdout;
  if v_bad<>0 then raise exception 'P1-02 AW9-12 quadratics QA: % reserve assessment items not holdout',v_bad; end if;
  ...
  if (select count(*) from private.exam_prep_question_content_meta m
    ...
    raise exception 'P1-02 AW9-12 quadratics QA: expected three isolated second-retest holdouts';
  end if;
end $$;

-- Recorded QA decisions for original, independently authored content.
...
where cv.id=m.content_version_id and cv.content_version='p1_aw09_12_quadratics_v1' and cv.status='draft';
...
set lifecycle_state=case when reserve_role='learning' then 'published' else 'reserve' end,
    ...
    updated_at=now()
from private.exam_prep_content_versions cv
where cv.id=m.content_version_id and cv.content_version='p1_aw09_12_quadratics_v1' and cv.status='approved';

update private.exam_prep_written_tasks w
set lifecycle_state='published'
from private.exam_prep_content_versions cv
where cv.id=w.content_version_id and cv.content_version='p1_aw09_12_quadratics_v1' and cv.status='approved' and w.lifecycle_state='approved';

update private.exam_prep_assessments a
set status='published',approved_at=coalesce(a.approved_at,now())
from private.exam_prep_content_versions cv
where cv.id=a.content_version_id and cv.content_version='p1_aw09_12_quadratics_v1' and cv.status='approved' and a.status='approved';

-- Publication trigger independently checks the full floor for every included skill.
update private.exam_prep_content_versions
set status='published',published_at=now()
where content_version='p1_aw09_12_quadratics_v1' and status='approved';

-- Final acceptance: the three skills are ready, AW9-12 remains closed until its full release is governed,
-- the already-green AW1-8 runway remains intact, and controlled beta remains fail-closed.
do $$
declare
  v_program bigint;
  v_skill text;
...
begin
  ...
  foreach v_skill in array array['P1-QUA-04','P1-QUA-05','P1-QUA-06'] loop
    if not private.exam_prep_skill_content_ready_v1(v_program,'P1',v_skill) then
      raise exception 'P1-02 AW9-12 quadratics final: skill not ready %',v_skill;
    end if;
  end loop;
  ...
  if v_p1_ready<>3 or v_p5_ready<>0 then
    raise exception 'P1-02 AW9-12 quadratics final: expected AW9 readiness P1=3 P5=0, got P1=% P5=%',v_p1_ready,v_p5_ready;
  end if;

  v_r1:=public.get_exam_prep_content_runway_v1(1::smallint);
  if coalesce((v_r1->>'hard_floor_green')::boolean,false) is not true
     or coalesce((v_r1->>'target_4w_green')::boolean,false) is not true
     or (v_r1#>>'{components,P1,ready_through_aw}')::int<>8
     or (v_r1#>>'{components,P5,ready_through_aw}')::int<>8 then
    raise exception 'P1-02 AW9-12 quadratics final: existing AW1-8 runway regressed: %',v_r1::text;
  end if;

  v_r9:=public.get_exam_prep_content_runway_v1(9::smallint);
  if coalesce((v_r9->>'hard_floor_green')::boolean,false)
     or coalesce((v_r9->>'target_4w_green')::boolean,false) then
    raise exception 'P1-02 AW9-12 quadratics final: AW9 must remain RED until all required P1/P5 skills are ready: %',v_r9::text;
  end if;
  ...
  if v_cfg.rollout_state<>'off' or v_cfg.core_enabled or v_cfg.ai_enabled or v_cfg.mentor_enabled or not v_cfg.kill_switch then
    raise exception 'P1-02 AW9-12 quadratics final: feature escaped fail-closed';
  end if;
  select count(*) into v_active from private.exam_prep_feature_entitlements where entitlement_status='active';
  if v_active<>0 then raise exception 'P1-02 AW9-12 quadratics final: active entitlement residue=%',v_active; end if;
end $$;
