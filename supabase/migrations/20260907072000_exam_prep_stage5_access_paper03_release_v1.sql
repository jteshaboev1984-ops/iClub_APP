-- Exam Prep Stage-5 access + third comparable full-paper release v1.
-- Stage 4 -> 5 requires the governed Stage-4 exit evaluator for the same component.
-- Paper03 is original iClub content and is released only at min_operational_stage=5.
-- Stage 6 remains locked; no readiness threshold is invented here.
...
insert into private.exam_prep_operational_stage_rules(
  ...
  'operational_stage_v3_2026_09_07','active',15.00,80.00,true,5,
  'Stage 0-4 semantics preserved from operational_stage_v2_2026_09_07. Stage 4 -> 5 is allowed only when private.exam_prep_stage4_exit_status_v1 is ready for the same learner/program/component. Stage 6 remains separately readiness-gated.'
);
...
begin
  ...
  if v_target>=2 and coalesce(new.coverage_pct,0)>=v_rule.stage2_to_3_min_coverage_pct then
    v_target:=3;
  end if;
  ...
  if new.operational_stage=0 then
    ...
    if coalesce(v_place.prerequisite_blocker_count,0)>0 then
      ...
      new.stage_hold_reason:=format('Stage 1 Foundation active; Stage 2 requires %s%% confirmed coverage or governed fast-track.',v_rule.stage1_to_2_min_coverage_pct);
    end if;
    ...
    new.stage_gate_status:='operational';
    new.stage_hold_reason:='Stage 4 Timed Consolidation active. Stage 5 requires two compatible strict full papers, non-worsening timing/unattempted evidence and closed or explicitly scheduled corrective work.';
  else
    new.stage_gate_status:='operational';
    new.stage_hold_reason:='Stage 5 Exam Readiness active. A third comparable full paper and the remaining readiness evidence are required; Stage 6 remains separately gated.';
  end if;
  return new;
end;
$$;
...
set paper03_release_status='approved',
    source_note=source_note || ' Release amendment 2026-09-07: Stage-5 operational access approved. The third original iClub full-paper form is released only behind min operational stage 5 so the learner can build the required last-three readiness window.',
    updated_at=now()
where status='active';

do $$ declare v_p1 jsonb; v_p5 jsonb; begin
  v_p1:=private.exam_prep_stage5_paper03_release_readiness_v1('P1');
  v_p5:=private.exam_prep_stage5_paper03_release_readiness_v1('P5');
  if not coalesce((v_p1->>'ready')::boolean,false) or not coalesce((v_p5->>'ready')::boolean,false) then
    raise exception 'Stage5 Paper03 release readiness failed P1=% P5=%',v_p1,v_p5;
  end if;
end $$;

update private.exam_prep_assessments
set status='published'
where assessment_key in ('p1_stage4_full_paper_03','p5_stage4_full_paper_03')
...
with target as (
  ...
  join private.exam_prep_component_paper_profiles p
    on p.program_version_id=cv.program_version_id
   and p.component_code=a.component_code
   and p.profile_version='9709_2026_2027_v1'
...
insert into private.exam_prep_timed_assessment_contracts(
  ...
  marks_available,fixed_time_limit_sec,strict_timing,comparison_scope,comparability_key,
  status,published_at,min_operational_stage
)
select id,paper_profile_id,
       case component_code when 'P1' then 'p1_stage5_full_paper_03_contract_v1' else 'p5_stage5_full_paper_03_contract_v1' end,
       'full_paper','official_full',official_total_marks,null,true,'full',
       case component_code when 'P1' then 'p1-full-paper-03-v1' else 'p5-full-paper-03-v1' end,
       'published',now(),5
from target;
...
begin
  select max_automatic_stage into v_max
  from private.exam_prep_operational_stage_rules where status='active';
  if v_max<>5 then raise exception 'Stage5 access release: expected max automatic stage 5 got=%',v_max; end if;

  select count(*) into v_published
  from private.exam_prep_assessments
  where assessment_key in ('p1_stage4_full_paper_03','p5_stage4_full_paper_03')
    and assessment_version='av1' and status='published';
  if v_published<>2 then raise exception 'Stage5 access release: Paper03 assessment publish mismatch=%',v_published; end if;
  ...
  where a.assessment_key in ('p1_stage4_full_paper_03','p5_stage4_full_paper_03')
    and c.status='published' and c.attempt_kind='full_paper'
    and c.timing_rule='official_full' and c.comparison_scope='full'
    and c.strict_timing and c.min_operational_stage=5;
  if v_contracts<>2 then raise exception 'Stage5 access release: Paper03 contract mismatch=%',v_contracts; end if;

  select count(*) into v_bad
  from private.exam_prep_timed_assessment_contracts c
  join private.exam_prep_assessments a on a.id=c.assessment_id
  where a.assessment_key in ('p1_stage4_full_paper_03','p5_stage4_full_paper_03')
    and private.exam_prep_timed_effective_min_stage_v1(c.attempt_kind,c.min_operational_stage::integer)<5;
  if v_bad<>0 then raise exception 'Stage5 access release: Paper03 stage leakage=%',v_bad; end if;

  if exists(select 1 from private.exam_prep_stage_states where operational_stage>5) then
    raise exception 'Stage5 access release: learner stage above approved ceiling';
  end if;
  ...
  if v_cfg.rollout_state<>'controlled_beta' or not v_cfg.core_enabled or v_cfg.ai_enabled or v_cfg.mentor_enabled or v_cfg.kill_switch then
    raise exception 'Stage5 access release: Core-only canary boundary drift';
  end if;
end $$;
