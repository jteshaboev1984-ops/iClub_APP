-- REPLAY RECONCILIATION FOR THE ALREADY-RELEASED 2026-09-23 PACKAGE.
-- Production received this migration through Supabase migration version 20260923111304
-- while its legacy v1 weekly generator had MD5 58247a59c0967848d21c5ab93cc9d647.
-- A clean historical migration replay deterministically yields the older equivalent
-- definition 0a2ab5a50e7f086b035960ccaccbc8eb. Normalize ONLY that known replay
-- state to the exact production-observed definition before executing the immutable
-- release package. Any third definition fails closed.

DO $weekly_replay_reconcile$
DECLARE v_hash text;
BEGIN
  v_hash:=md5(pg_get_functiondef(
    'public.generate_exam_prep_weekly_plan_safe_v1(text,text)'::regprocedure
  ));
  IF v_hash='58247a59c0967848d21c5ab93cc9d647' THEN
    RETURN;
  ELSIF v_hash<>'0a2ab5a50e7f086b035960ccaccbc8eb' THEN
    RAISE EXCEPTION 'weekly_release_replay_unknown_legacy_v1_hash_%',v_hash;
  END IF;
END
$weekly_replay_reconcile$;

CREATE OR REPLACE FUNCTION public.generate_exam_prep_weekly_plan_safe_v1(
  p_component_code text,
  p_recovery_mode text DEFAULT 'normal'::text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DO $weekly_replay_verify$
BEGIN
  IF md5(pg_get_functiondef(
      'public.generate_exam_prep_weekly_plan_safe_v1(text,text)'::regprocedure
     ))<>'58247a59c0967848d21c5ab93cc9d647'
  THEN
    RAISE EXCEPTION 'weekly_release_replay_normalization_failed';
  END IF;
END
$weekly_replay_verify$;
