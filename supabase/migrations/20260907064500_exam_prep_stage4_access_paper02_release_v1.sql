...
do $$
declare v_p1 jsonb; v_p5 jsonb; begin
  ...
  if not coalesce((v_p1->>'ready')::boolean,false) or not coalesce((v_p5->>'ready')::boolean,false) then
    raise exception 'Stage4 Paper02 release readiness failed P1=% P5=%',v_p1,v_p5;
  end if;
end $$;
...
with target as (
  ...
  join private.exam_prep_program_versions pv on pv.id=a.program_version_id
  join private.exam_prep_component_paper_profiles p
    on p.program_version_id=pv.id and p.component_code=a.component_code
...
do $$
declare
  v_max smallint; v_published int; v_contracts int; v_bad int; v_cfg private.exam_prep_feature_config%rowtype;
begin
  ...
  select count(*) into v_published from private.exam_prep_assessments
  where assessment_key in ('p1_stage4_full_paper_02','p5_stage4_full_paper_02') and assessment_version='av1' and status='published';
  ...
  where a.assessment_key in ('p1_stage4_full_paper_02','p5_stage4_full_paper_02')
    and c.status='published' and c.attempt_kind='full_paper' and c.timing_rule='official_full'
    and c.comparison_scope='full' and c.strict_timing and c.min_operational_stage=4;
  ...
  join private.exam_prep_assessments a on a.id=c.assessment_id
  where a.assessment_key in ('p1_stage4_full_paper_02','p5_stage4_full_paper_02')
    and private.exam_prep_timed_effective_min_stage_v1(c.attempt_kind,c.min_operational_stage::integer)<4;
  if v_bad<>0 then raise exception 'Stage4 access release: Paper02 stage leakage=%',v_bad; end if;

  if exists(select 1 from private.exam_prep_stage_states where operational_stage>4) then
    raise exception 'Stage4 access release: learner stage above approved ceiling';
  end if;
  ...
  if v_cfg.rollout_state<>'controlled_beta' or not v_cfg.core_enabled or v_cfg.ai_enabled or v_cfg.mentor_enabled or v_cfg.kill_switch then
    raise exception 'Stage4 access release: Core-only canary boundary drift';
  end if;
end $$;
