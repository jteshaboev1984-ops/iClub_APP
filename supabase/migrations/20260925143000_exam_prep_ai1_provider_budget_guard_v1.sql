-- AI-1 provider budget/concurrency guard v1.
-- Additive and dormant-safe: no learner enablement, no academic-state mutation.
-- Provider admission is serialized in Postgres so limits hold across Edge Function instances.

alter table private.exam_prep_ai_policy
  add column if not exists max_daily_provider_cost_usd numeric(10,4) not null default 0.5000
    check (max_daily_provider_cost_usd > 0 and max_daily_provider_cost_usd <= 100),
  add column if not exists max_user_daily_provider_cost_usd numeric(10,4) not null default 0.0500
...
create table if not exists private.exam_prep_ai_provider_leases (
...
create index if not exists exam_prep_ai_provider_leases_active_idx
  on private.exam_prep_ai_provider_leases(status, expires_at)
  where status='active';

create index if not exists exam_prep_ai_provider_leases_user_day_idx
  on private.exam_prep_ai_provider_leases(user_id, created_at);

alter table private.exam_prep_ai_provider_leases enable row level security;
revoke all on private.exam_prep_ai_provider_leases from public, anon, authenticated;
grant all on private.exam_prep_ai_provider_leases to service_role;

create or replace function public.reserve_exam_prep_ai_provider_call_service_v1(
  p_request_id uuid,
  p_user_id uuid,
...
declare
  ...
  v_existing private.exam_prep_ai_provider_leases%rowtype;
...
begin
  ...
  -- Single global admission lock. Provider traffic is intentionally small in AI-1,
  -- and this prevents cross-instance races in concurrency and spend checks.
  perform pg_advisory_xact_lock(202609251);
  ...
  update private.exam_prep_ai_provider_leases
  set status='expired',
  ...
  insert into private.exam_prep_ai_provider_leases(
...
revoke all on function public.reserve_exam_prep_ai_provider_call_service_v1(uuid,uuid,numeric)
  from public, anon, authenticated;
grant execute on function public.reserve_exam_prep_ai_provider_call_service_v1(uuid,uuid,numeric)
  to service_role;

create or replace function public.finalize_exam_prep_ai_provider_call_service_v1(
  p_request_id uuid,
  p_status text,
...
as $$
declare
  v_row private.exam_prep_ai_provider_leases%rowtype;
  v_status text;
...
revoke all on function public.finalize_exam_prep_ai_provider_call_service_v1(uuid,text,numeric)
  from public, anon, authenticated;
grant execute on function public.finalize_exam_prep_ai_provider_call_service_v1(uuid,text,numeric)
  to service_role;
