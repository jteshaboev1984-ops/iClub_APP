-- Production-applied Supabase migration 20260903051124 / extend_practice_resume_time_safe_v4.
-- Resume now carries authoritative saved per-question time for already answered Practice questions.

drop function if exists public.get_practice_session_resume_safe_v4(bigint);

create function public.get_practice_session_resume_safe_v4(p_session_id bigint)
returns table(
  order_no integer,
  id bigint,
  subject_id bigint,
...
as $function$
  select
    ...
    case when a.session_id is not null then a.time_spent else null end as saved_time_spent,
