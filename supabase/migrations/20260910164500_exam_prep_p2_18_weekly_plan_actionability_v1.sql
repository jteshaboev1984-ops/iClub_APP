-- P2-18: keep a normal weekly plan actionable.
-- A plan has at most three priorities and, in normal mode, at most one visible retest slot.
-- Extra future correction-retest reminders must not crowd out current correction work.
-- Recovery-mode planner behavior is intentionally preserved.
...
begin
  ...
  -- Normal mode: one explicit retest priority is enough. Regeneration exposes the next one.
  if coalesce(v_base->>'recovery_mode','normal')='normal' then
    ...
    where i.plan_id=v_plan and i.item_type='retest'
      and i.priority_order<>(
        select min(k.priority_order)
        from private.exam_prep_weekly_plan_items k
        where k.plan_id=v_plan and k.item_type='retest'
      );

    -- Refill any freed slot with current actionable correction work before adding optional work.
    for v_corr in
      ...
      where c.user_id=v_uid and c.component_code=p_component_code
        ...
        and not exists(
          ...
          where i.plan_id=v_plan and i.correction_case_id=c.id
        )
    ...
    loop
      ...
      where not exists(
        select 1 from private.exam_prep_weekly_plan_items i
        where i.plan_id=v_plan and i.priority_order=gs
      );
      ...
      insert into private.exam_prep_weekly_plan_items(
        ...
        v_plan,v_replace,'correction',v_corr.skill_code,v_corr.id,
