-- P1-01 weekly governance recorder v2.
-- Additive/versioned: v1 remains intact for audit compatibility.
-- v2 records the corrected weekly_snapshot_v2 and treats content-runway,
-- denominator and legacy-credit violations as Core hard blockers.
-- Metrics still never self-authorize a release decision: an explicit governed
-- reviewer and explicit per-service decisions are required.

begin;

create or replace function public.record_exam_prep_beta_weekly_review_v2(
  p_cohort_key text,
...
begin
  ...
  v_core_blockers:=
    ...
    coalesce((v_snapshot#>>'{hard_blockers,stage_denominator_mismatches}')::int,0)+
    coalesce((v_snapshot#>>'{hard_blockers,legacy_reference_credit_violations}')::int,0)+
    coalesce((v_snapshot#>>'{hard_blockers,content_runway_hard_floor_red}')::int,0);
  ...
  insert into private.exam_prep_audit_events(
    ...
    'math_as_p1_p5',p_reviewer_user_id,'beta_governance','beta_weekly_review_recorded_v2',
    'private.exam_prep_beta_weekly_reviews',v_review::text,
    jsonb_build_object(
      'cohort_key',p_cohort_key,
      'review_no',p_review_no,
      'overall_decision',p_overall_decision,
      'monitoring_semantics_version',coalesce(v_snapshot->>'monitoring_semantics_version','unknown'),
      'core_hard_blocker_count',v_core_blockers,
      'mentor_hard_blocker_count',v_mentor_blockers
    )
  );
  ...
  return jsonb_build_object(
    ...
    'review_no',p_review_no,
    'overall_decision',p_overall_decision,
    'snapshot_hash',md5(v_snapshot::text),
    'monitoring_semantics_version',coalesce(v_snapshot->>'monitoring_semantics_version','unknown')
  );
end;
$$;

revoke all on function public.record_exam_prep_beta_weekly_review_v2(
...
-- Contract-only checks; no beta rows are created or modified.
do $$
begin
  if to_regprocedure('public.get_exam_prep_beta_weekly_snapshot_v2(text,timestamptz)') is null then
    raise exception 'P1-01 weekly review v2 requires weekly snapshot v2';
  end if;
  if has_function_privilege('anon','public.record_exam_prep_beta_weekly_review_v2(text,smallint,timestamptz,text,text,text,text,text,text,text,text,uuid)','EXECUTE') then
    raise exception 'P1-01 weekly review v2 anon execution must remain revoked';
  end if;
  if has_function_privilege('authenticated','public.record_exam_prep_beta_weekly_review_v2(text,smallint,timestamptz,text,text,text,text,text,text,text,text,uuid)','EXECUTE') then
    raise exception 'P1-01 weekly review v2 authenticated execution must remain revoked';
  end if;
end;
$$;
