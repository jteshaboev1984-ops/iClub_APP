...
create unique index if not exists exam_prep_program_versions_one_active_idx
  on private.exam_prep_program_versions(program_key) where status = 'active';
...
create table if not exists private.exam_prep_exam_profiles (
  ...
  check (total_student_hours_available is null or mathematics_hours_budget is null or mathematics_hours_budget <= total_student_hours_available),
...
-- 3) Mentor Care service lifecycle. Entitlement is not operational assignment.
create table if not exists private.exam_prep_mentor_service_status (
  ...
  service_status text not null default 'not_entitled'
    check (service_status in ('not_entitled','entitled_waitlist','assigned_active','assigned_paused','ended')),
...
-- 4) Component-scoped mentor assignment permission anchor.
...
-- RLS/grants: authenticated may read only own planning/service anchors; no direct writes.
...
create policy exam_prep_exam_profiles_owner_read on private.exam_prep_exam_profiles
for select to authenticated using ((select auth.uid()) = user_id);

drop policy if exists exam_prep_mentor_service_status_owner_read on private.exam_prep_mentor_service_status;
create policy exam_prep_mentor_service_status_owner_read on private.exam_prep_mentor_service_status
for select to authenticated using ((select auth.uid()) = learner_user_id);

drop policy if exists exam_prep_mentor_assignments_learner_read on private.exam_prep_mentor_assignments;
create policy exam_prep_mentor_assignments_learner_read on private.exam_prep_mentor_assignments
for select to authenticated using ((select auth.uid()) = learner_user_id);

-- Generic private audit trigger; never directly executable by client roles.
...
-- Existing P0-04 capability state is audited from now on too.
drop trigger if exists exam_prep_feature_config_audit_v1 on private.exam_prep_feature_config;
create trigger exam_prep_feature_config_audit_v1 after insert or update or delete on private.exam_prep_feature_config
...
create trigger exam_prep_feature_entitlements_audit_v1 after insert or update or delete on private.exam_prep_feature_entitlements
for each row execute function private.exam_prep_audit_row_change_v1();

drop trigger if exists exam_prep_program_versions_audit_v1 on private.exam_prep_program_versions;
create trigger exam_prep_program_versions_audit_v1 after insert or update or delete on private.exam_prep_program_versions
for each row execute function private.exam_prep_audit_row_change_v1();

drop trigger if exists exam_prep_exam_profiles_audit_v1 on private.exam_prep_exam_profiles;
create trigger exam_prep_exam_profiles_audit_v1 after insert or update or delete on private.exam_prep_exam_profiles
for each row execute function private.exam_prep_audit_row_change_v1();

drop trigger if exists exam_prep_mentor_service_status_audit_v1 on private.exam_prep_mentor_service_status;
create trigger exam_prep_mentor_service_status_audit_v1 after insert or update or delete on private.exam_prep_mentor_service_status
for each row execute function private.exam_prep_audit_row_change_v1();

drop trigger if exists exam_prep_mentor_assignments_audit_v1 on private.exam_prep_mentor_assignments;
create trigger exam_prep_mentor_assignments_audit_v1 after insert or update or delete on private.exam_prep_mentor_assignments
for each row execute function private.exam_prep_audit_row_change_v1();

-- Upgrade P0-04 capability RPC with real operational assignment gating.
...
as $$
  with caller as (
    ...
    select
      caller.user_id, cfg.program_key, cfg.rollout_state, cfg.core_enabled, cfg.ai_enabled,
      cfg.mentor_enabled, cfg.kill_switch,
  ...
  select
    ...
    coalesce(e.user_id is not null and not e.kill_switch and e.core_enabled and e.mentor_enabled and e.rollout_state <> 'off' and e.ent_core and e.ent_mentor and e.service_assigned_active and e.has_active_assignment, false),
    coalesce(e.user_id is not null and not e.kill_switch and e.core_enabled and e.mentor_enabled and e.rollout_state <> 'off' and e.ent_core and e.ent_mentor and e.service_assigned_active and e.has_active_assignment, false),
