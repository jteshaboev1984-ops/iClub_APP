...
--
-- This file intentionally contains the destructive-permission cutover separately from P0-02.

begin;

-- 1. Remove direct public row access to questions (answer-key leak).
drop policy if exists questions_public_read on public.questions;

-- 2. Protect Tour membership. Active/upcoming Tour question IDs must not be enumerable.
drop policy if exists tour_questions_public_read on public.tour_questions;

-- 3. Remove direct browser writes to Tour attempts/answers.
-- Keep authenticated SELECT on tour_attempts for existing ratings until ratings is migrated.
...
-- 4. Public-safe view remains useful for non-competitive diagnostic/learning flows,
-- but must never disclose questions belonging to a Tour whose official window has not ended.
