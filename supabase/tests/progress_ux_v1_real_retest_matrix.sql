-- Disposable integration test: verified remediation provenance followed by the REAL
-- finalized-session retest trigger. Projection must reflect pass/fail without
-- awarding mastery or weekly completion from an unbound correction action.
...
DECLARE
  ...
  auth_id uuid; rt_id uuid; v_session_id uuid; v_response_id uuid;
...
BEGIN
  ...
  VALUES(u,prog,'P1',1,1,'superseded','Verified-remediation projection source')
  RETURNING id INTO source_plan;
  ...
  IF (public.ensure_exam_prep_weekly_goals_safe_v1('P1')->>'created')::int<>2 THEN
    RAISE EXCEPTION 'Stable correction goals were not anchored';
  END IF;

  -- Seed existing *server-confirmed* remediation actions, not the remediation
  -- evaluator itself (which has a separate academic regression). Each action is
  -- backed by an actual finalized, authorized session for its ORIGINAL plan item.
  -- An orphan remediation_completed row must never earn weekly credit.
  FOR rec IN SELECT * FROM (VALUES
    (pass_case,'P1-QUA-01'::text,1),
    (fail_case,'P1-QUA-02'::text,2)
  ) AS x(case_id,skill_code,priority_order) LOOP
    ...
    IF ass IS NULL THEN RAISE EXCEPTION 'Learning fixture missing for %',rec.skill_code; END IF;
    ...
    VALUES(u,ass,'P1','learning','issued',clock_timestamp()+interval '1 hour',
      'Isolated previously-verified remediation provenance',true,source_plan,rec.priority_order)
    ...
    VALUES(v_session_id,auth_id,u,prog,content_id,ass,ass_ver,'P1','learning','active',
      'px-remediation-'||rec.priority_order::text,1);
    ...
    VALUES(v_session_id,1,'question',q,rec.skill_code,reserve_role,false,meta,checksum,
      ass_ver||'|px-remediation-'||rec.priority_order::text);
    ...
    VALUES(v_session_id,1,u,'px-remediation-answer-'||rec.priority_order::text,
      'machine','isolated-fixture',true,'progress_ux_remediation_provenance',1000)
    ...
    VALUES(u,'P1',rec.skill_code,v_session_id,v_response_id,'learning','app_verified',true,
      jsonb_build_object('isolated_fixture','prior_remediation_provenance'),ass_ver||'|px-remediation');
    UPDATE private.exam_prep_sessions
      SET status='finalized',finalized_at=clock_timestamp(),last_activity_at=clock_timestamp(),
          finalize_idempotency_key='px-remediation-final-'||rec.priority_order::text
    WHERE id=v_session_id;
    -- A previously verified academic decision is a fixture here; the real
    -- academic retest evaluation below is performed by the production trigger.
    ...
    VALUES(rec.case_id,u,'P1',rec.skill_code,'remediation_completed',v_session_id,
      jsonb_build_object('fixture','previously_verified_remediation'));
  END LOOP;
  result:=public.get_exam_prep_weekly_progress_safe_v1('P1');
  IF (result->>'completed_goals')::int<>2
     OR (result->>'finalized_study_sessions')::int<>2
     OR result->'goals'->0->>'status'<>'waiting_retest'
     OR result->'goals'->1->>'status'<>'waiting_retest' THEN
    RAISE EXCEPTION 'Bound historical remediation did not receive correct weekly credit: %',result;
  END IF;
  ...
  FOR rec IN SELECT * FROM (VALUES
  ...
  ) AS x(case_id,skill_code,should_pass,priority_order) LOOP
    SELECT a.id,a.content_version_id,a.assessment_version,ai.question_id,ai.reserve_role,
           m.id,m.question_snapshot_md5
    INTO ass,content_id,ass_ver,q,reserve_role,meta,checksum
    FROM private.exam_prep_assessments a
    ...
    WHERE a.component_code='P1' AND a.assessment_type='retest' AND a.status='published'
      AND ai.primary_skill_code=rec.skill_code AND ai.question_id IS NOT NULL
      AND NOT EXISTS(SELECT 1 FROM private.exam_prep_assessment_items other
    ...
    -- Invoke the existing REAL retest reconciliation trigger.
    UPDATE private.exam_prep_sessions
      SET status='finalized',finalized_at=clock_timestamp(),last_activity_at=clock_timestamp(),
          finalize_idempotency_key='px-real-retest-final-'||rec.priority_order::text
    WHERE id=v_session_id;
    IF rec.should_pass THEN
      IF NOT EXISTS(SELECT 1 FROM private.exam_prep_correction_cases c
        WHERE c.id=rec.case_id AND c.status='resolved')
        OR NOT EXISTS(SELECT 1 FROM private.exam_prep_correction_actions ca
          WHERE ca.correction_case_id=rec.case_id AND ca.action_type='retest_passed'
            AND ca.session_id=v_session_id) THEN
        RAISE EXCEPTION 'Real passing retest did not resolve/log pass'; END IF;
    ELSE
      IF NOT EXISTS(SELECT 1 FROM private.exam_prep_correction_cases c
        WHERE c.id=rec.case_id AND c.status='reopened')
        OR NOT EXISTS(SELECT 1 FROM private.exam_prep_correction_actions ca
          WHERE ca.correction_case_id=rec.case_id AND ca.action_type='retest_failed'
            AND ca.session_id=v_session_id) THEN
  ...
  result:=public.get_exam_prep_weekly_progress_safe_v1('P1');
  SELECT value INTO goal FROM jsonb_array_elements(result->'goals') value
    WHERE (value->>'priority_order')::int=1;
  IF goal->>'status'<>'completed' OR goal->>'correction_open'<>'false'
     OR (goal->>'finalized_sessions')::int<>1 THEN
    RAISE EXCEPTION 'Passing real retest projected incorrectly: %',result; END IF;
  SELECT value INTO goal FROM jsonb_array_elements(result->'goals') value
    WHERE (value->>'priority_order')::int=2;
  IF goal->>'status'<>'needs_rework' OR goal->>'correction_open'<>'true'
     OR (goal->>'finalized_sessions')::int<>1 THEN
    RAISE EXCEPTION 'Failed real retest projected incorrectly: %',result; END IF;
  IF (result->>'completed_goals')::int<>2
     OR (result->>'finalized_study_sessions')::int<>4
     OR (result->>'open_corrections')::int<>1 THEN
    RAISE EXCEPTION 'Real retest changed historical weekly commitment or session totals: %',result;
  END IF;
  RAISE NOTICE 'Progress UX real retest PASS: authorized history, actual pass/fail trigger, weekly facts retained';
END;
$matrix$;
ROLLBACK;
DO $$ BEGIN
  IF EXISTS(SELECT 1 FROM public.users WHERE first_name='ProgressUXRealRetest') THEN
    RAISE EXCEPTION 'Real retest fixture leaked into database';
  END IF;
