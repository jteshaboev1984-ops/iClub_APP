...
BEGIN
  ...
  if v_bad<>20 then
    raise exception 'AI-4 Chapter 11 diagnostic row count incorrect: %',v_bad;
  end if;
  ...
  from (
    ...
    where q.id in (1028,1029,1079,1121,1133)
    group by q.id
  ) x
  where mapping_count<>4 or correct_count<>1 or localized_wrong_count<>3;

  if v_bad<>0 then
    raise exception 'AI-4 Chapter 11 diagnostics incomplete: %',v_bad;
  end if;
  ...
  if v_bad<>0 then
    raise exception 'AI-4 Chapter 11 correctness disagrees with server answer key: %',v_bad;
  end if;

  select count(*) into v_sentinel
  from public.question_answer_diagnostics
  where question_id=1081
    and quality_status='published'
    and feedback_en='existing sentinel en';

  if v_sentinel<>1 then
    raise exception 'AI-4 Chapter 11 batch mutated existing q1081 diagnostics: %',v_sentinel;
  end if;
END
$$;
...
\echo 'AI-4 Economics Practice 1 Chapter 11 diagnostic matrix: GREEN'
