...
CREATE TEMP TABLE ai2_provider_people(person_key text primary key,user_id uuid not null) ON COMMIT DROP;
...
BEGIN
  ...
  VALUES
    (v_user1,'authenticated','authenticated','ai2-provider-1-'||replace(v_user1::text,'-','')||'@invalid.example',now(),now(),false,false),
    (v_user2,'authenticated','authenticated','ai2-provider-2-'||replace(v_user2::text,'-','')||'@invalid.example',now(),now(),false,false);

  INSERT INTO public.users(id,first_name,last_name,language_code,created_at,must_change_password)
  VALUES
    (v_user1,'AI2','Provider One','en',now(),false),
    (v_user2,'AI2','Provider Two','en',now(),false);

  INSERT INTO ai2_provider_people VALUES('u1',v_user1),('u2',v_user2);
END
$$;
...
DO $$
DECLARE
  v_u1 uuid:=(SELECT user_id FROM ai2_provider_people WHERE person_key='u1');
  v_req1 uuid:=gen_random_uuid();
  v_req2 uuid:=gen_random_uuid();
  v jsonb;
BEGIN
  v:=public.reserve_practice_ai_provider_call_service_v1(v_req1,v_u1,0.0060);
  IF coalesce((v->>'allowed')::boolean,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'AI-2 first reservation should succeed: %',v;
  END IF;

  v:=public.reserve_practice_ai_provider_call_service_v1(v_req1,v_u1,0.0060);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'duplicate_request' THEN
    RAISE EXCEPTION 'AI-2 duplicate request was not blocked: %',v;
  END IF;

  v:=public.reserve_practice_ai_provider_call_service_v1(v_req2,v_u1,0.0040);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'provider_concurrency_limit' THEN
    RAISE EXCEPTION 'AI-2 concurrency limit failed: %',v;
  END IF;
  ...
  IF coalesce((v->>'ok')::boolean,false) IS NOT TRUE OR v->>'status'<>'completed' THEN
    RAISE EXCEPTION 'AI-2 provider completion failed: %',v;
  END IF;

  v:=public.reserve_practice_ai_provider_call_service_v1(v_req2,v_u1,0.0040);
  IF coalesce((v->>'allowed')::boolean,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'AI-2 reservation after completion should succeed: %',v;
  END IF;

  v:=public.finalize_practice_ai_provider_call_service_v1(v_req2,'completed',0.0030);
  IF coalesce((v->>'ok')::boolean,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'AI-2 second completion failed: %',v;
  END IF;
END
$$;

UPDATE private.practice_ai_policy SET max_daily_requests=2,updated_at=now() WHERE id=1;
...
DECLARE
  ...
  v jsonb;
BEGIN
  v:=public.reserve_practice_ai_provider_call_service_v1(gen_random_uuid(),v_u1,0.0010);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'daily_request_limit' THEN
    RAISE EXCEPTION 'AI-2 daily provider-call limit failed: %',v;
  END IF;
END
$$;
...
DO $$
DECLARE
  v_u2 uuid:=(SELECT user_id FROM ai2_provider_people WHERE person_key='u2');
  v_req uuid:=gen_random_uuid();
  v jsonb;
BEGIN
  v:=public.reserve_practice_ai_provider_call_service_v1(v_req,v_u2,0.0050);
  IF coalesce((v->>'allowed')::boolean,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'AI-2 user-cost seed reservation failed: %',v;
  END IF;
  PERFORM public.finalize_practice_ai_provider_call_service_v1(v_req,'completed',0.0050);

  v:=public.reserve_practice_ai_provider_call_service_v1(gen_random_uuid(),v_u2,0.0020);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'user_daily_cost_limit' THEN
    RAISE EXCEPTION 'AI-2 per-user daily cost limit failed: %',v;
  END IF;

  v:=public.reserve_practice_ai_provider_call_service_v1(gen_random_uuid(),v_u2,0.0070);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'request_cost_limit' THEN
    RAISE EXCEPTION 'AI-2 per-request cost limit failed: %',v;
  END IF;
END
$$;
...
DECLARE
  ...
  v jsonb;
BEGIN
  v:=public.reserve_practice_ai_provider_call_service_v1(gen_random_uuid(),v_u1,0.0030);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'global_daily_cost_limit' THEN
    RAISE EXCEPTION 'AI-2 global daily cost limit failed: %',v;
  END IF;
END
$$;
...
BEGIN
  ...
  IF v_snapshot#>>'{policy,max_daily_provider_cost_usd}' IS NULL
    ...
    RAISE EXCEPTION 'AI-2 operational snapshot missing provider fields: %',v_snapshot;
  END IF;
END
$snap$;
...
BEGIN
  ...
  FROM auth.users
  WHERE email like 'ai2-provider-%@invalid.example';
  IF v_count<>0 THEN
    RAISE EXCEPTION 'AI-2 rollback left synthetic users=%',v_count;
  END IF;

  SELECT count(*) INTO v_count FROM private.practice_ai_provider_leases;
  IF v_count<>0 THEN
    RAISE EXCEPTION 'AI-2 rollback left provider lease residue=%',v_count;
  END IF;
END
$$;
