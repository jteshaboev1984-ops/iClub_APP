...
-- BY DEFAULT identity mirrors production behavior: fixed legacy IDs may be
-- seeded, while later P0-08 content inserts may omit id and receive one safely.
...
-- Move the sequence above the fixed legacy range so test-only P0-08 inserts can
-- never collide with the explicit P0-07 IDs.
select setval(pg_get_serial_sequence('public.questions','id'), 100000, true);
