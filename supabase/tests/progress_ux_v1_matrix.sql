-- Exam Prep Progress UX runtime test. EPHEMERAL POSTGRES ONLY; all data rolls back.
\set ON_ERROR_STOP on
DO $$ BEGIN
  IF current_setting('progress_ux.isolated_db',true) IS DISTINCT FROM 'true' THEN
    RAISE EXCEPTION 'PROGRESS UX REFUSED: progress_ux.isolated_db=true required';
  END IF;
END $$;
BEGIN;
...
DO $$ DECLARE v_program bigint; BEGIN
  ...
  IF v_program IS NULL THEN RAISE EXCEPTION 'No active canonical program in isolated bootstrap'; END IF;
...
CREATE TEMP TABLE px_cases(ord int, case_id uuid, PRIMARY KEY(ord)) ON COMMIT DROP;
...
WITH inserted AS (
 ...
 RETURNING id
) INSERT INTO px_plans SELECT 'P1',1,id FROM inserted;
...
((SELECT plan_id FROM px_plans WHERE component_code='P1' AND version_no=1),2,'learning','P1-COO-02',null,'BUILD_FIRST_COVERAGE'),
((SELECT plan_id FROM px_plans WHERE component_code='P1' AND version_no=1),3,'learning','P1-TRI-01',null,'BUILD_FIRST_COVERAGE');
WITH inserted AS (
 INSERT INTO private.exam_prep_weekly_plans(user_id,program_version_id,component_code,active_week_no,plan_version,status,policy_note)
 SELECT p.user_id,e.program_version_id,'P1',1,2,'active','isolated regenerated plan'
 FROM px_people p JOIN private.exam_prep_exam_profiles e ON e.user_id=p.user_id WHERE p.ord=1
 RETURNING id
) INSERT INTO px_plans SELECT 'P1',2,id FROM inserted;
INSERT INTO private.exam_prep_weekly_plan_items(plan_id,priority_order,item_type,skill_code,correction_case_id,action_code)
VALUES
...
WITH inserted AS (
 ...
 FROM px_people p JOIN private.exam_prep_exam_profiles e ON e.user_id=p.user_id WHERE p.ord=1
 RETURNING id
) INSERT INTO px_plans SELECT 'P5',1,id FROM inserted;
...
GRANT SELECT ON TABLE px_people,px_cases,px_plans TO authenticated;

DO $$ DECLARE v_one uuid; v_two uuid; v_result jsonb; v_after jsonb; v_before jsonb; v_ids text[]; BEGIN
 ...
 IF v_before->>'plan_available'<>'false' OR jsonb_array_length(v_before->'goals')<>0 THEN
  RAISE EXCEPTION 'No snapshots must not fabricate 0/3'; END IF;
 v_result:=public.ensure_exam_prep_weekly_goals_safe_v1('P1');
 IF (v_result->>'created')::int<>3 THEN RAISE EXCEPTION 'Expected exactly 3 first-plan goals: %',v_result; END IF;
 v_after:=public.get_exam_prep_weekly_progress_safe_v1('P1');
 IF v_after->>'component_code'<>'P1' OR (v_after->>'completed_goals')::int<>0
    OR jsonb_array_length(v_after->'goals')<>3 THEN
 ...
 IF v_after->'goals'->0->>'action_priority_order'<>'1' OR
  ...
  RAISE EXCEPTION 'Current action binding/replan displacement incorrect: %',v_after->'goals'; END IF;
 SELECT array_agg(x->>'goal_id' ORDER BY (x->>'priority_order')::int) INTO v_ids
 FROM jsonb_array_elements(v_after->'goals') x;
 v_result:=public.ensure_exam_prep_weekly_goals_safe_v1('P1');
 IF (v_result->>'created')::int<>0 THEN RAISE EXCEPTION 'Snapshot repeated inserts'; END IF;
 v_after:=public.get_exam_prep_weekly_progress_safe_v1('P1');
 IF v_ids IS DISTINCT FROM (SELECT array_agg(x->>'goal_id' ORDER BY (x->>'priority_order')::int)
     FROM jsonb_array_elements(v_after->'goals') x) THEN RAISE EXCEPTION 'Repeated read moved stable goals'; END IF;
 v_result:=public.ensure_exam_prep_weekly_goals_safe_v1('P5');
 IF (v_result->>'created')::int<>1 THEN RAISE EXCEPTION 'P5 first plan not scoped'; END IF;
 ...
 IF jsonb_array_length(v_result->'goals')<>1 OR v_result->'goals'->0->>'component_code'<>'P5' THEN
   RAISE EXCEPTION 'P5 component firewall broken: %',v_result; END IF;
 -- The second eligible learner has no goals; neither request may read learner one.
 PERFORM set_config('request.jwt.claim.sub',v_two::text,true);
 v_result:=public.get_exam_prep_weekly_progress_safe_v1('P1');
 IF v_result->>'plan_available'<>'false' OR jsonb_array_length(v_result->'goals')<>0
    OR (v_result->>'finalized_study_sessions')::int<>0 THEN
    RAISE EXCEPTION 'Cross-user data leak: %',v_result; END IF;
 v_result:=public.ensure_exam_prep_weekly_goals_safe_v1('P1');
 ...
 BEGIN
   ...
   RAISE EXCEPTION 'Unauthenticated read unexpectedly succeeded';
 EXCEPTION WHEN sqlstate '28000' THEN NULL; END;
 RAISE NOTICE 'Progress UX isolated matrix: PASS (auth, no fake progress, stable goal IDs, plan versioning, P1/P5 firewall, idempotence, cross-user isolation)';
END $$;

-- Transaction must erase all synthetic users, plans, goals, configuration and entitlement changes.
ROLLBACK;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM public.users WHERE first_name='ProgressUXSynthetic') THEN
  RAISE EXCEPTION 'Rollback left synthetic user residue';
 END IF;
