...
BEGIN
  ...
  where question_id in (1102,1109,1122,1128,1131) and quality_status='published';
  if v_total<>20 then
    raise exception 'AI-4 Chapter 4 diagnostic row count incorrect: %',v_total;
  end if;
  ...
  select count(*) into v_bad from (
    ...
    where q.id in (1102,1109,1122,1128,1131)
    group by q.id
  ) x
  where x.mapping_count<>4 or x.correct_mapping_count<>1 or x.safe_wrong_mapping_count<>3;
  if v_bad<>0 then
    raise exception 'AI-4 Chapter 4 diagnostics incomplete for target questions: %',v_bad;
  end if;
  ...
  if v_bad<>0 then
    raise exception 'AI-4 Chapter 4 diagnostic correctness disagrees with server answer key: %',v_bad;
  end if;

  select count(*) into v_sentinel
  from public.question_answer_diagnostics
  where question_id=1066 and quality_status='published' and feedback_en='existing sentinel en';
  if v_sentinel<>1 then
    raise exception 'AI-4 Chapter 4 batch mutated existing Chapter 3 diagnostics: %',v_sentinel;
  end if;
END $$;
ROLLBACK;
\echo 'AI-4 Economics Practice 1 Chapter 4 diagnostic matrix: GREEN'
