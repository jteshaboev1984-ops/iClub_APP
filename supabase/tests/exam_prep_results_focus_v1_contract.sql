...
begin
  ...
  v_ensure:=pg_get_functiondef('public.ensure_exam_prep_stable_weekly_plan_safe_v1(text)'::regprocedure);
  if position('exam_prep_balance_new_normal_plan_v1' in v_ensure)=0 then
    raise exception 'results_focus_v1 new-week balance not wired';
  end if;
end $$;
rollback;
