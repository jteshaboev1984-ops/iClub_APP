...
SELECT
  ...
  (SELECT count(*) FROM private.exam_prep_readiness_states) AS readiness_count;
...
BEGIN
  -- P2-74 must use a dedicated synthetic identity with explicit provenance.
  PERFORM private.register_exam_prep_synthetic_validation_run_v1(
    v_run,
    'p274-ai-shadow-safety-v1',
    repeat('7',40),
    'p2-74',
    27401,
    'ai_shadow',
  ...
  VALUES(
    ...
    'exam-prep-sv-p274-'||replace(v_uid::text,'-','')||'@invalid.example',
    now(),now(),false,false
  );
  ...
  INSERT INTO private.exam_prep_synthetic_identities(
    ...
    v_uid,v_run,'learner','SVF-P274-AI-SHADOW','active','p2-74-ai-shadow-proof',
    'Dedicated P2-74 synthetic learner. Never a real beta member.'
  );

  INSERT INTO p274_people(person_key,user_id) VALUES('learner',v_uid);

  -- A cohort-less entitlement is the dedicated synthetic-harness path.
...
-- Browser roles must not gain direct access to private AI state or service-only helpers.
DO $$
BEGIN
  ...
  IF has_function_privilege('anon','public.get_exam_prep_ai_guard_v1(text,text,text,integer)','EXECUTE') THEN
    RAISE EXCEPTION 'P2-74 anon can execute learner AI guard';
  END IF;
  IF NOT has_function_privilege('authenticated','public.get_exam_prep_ai_guard_v1(text,text,text,integer)','EXECUTE') THEN
    RAISE EXCEPTION 'P2-74 authenticated learner AI guard missing';
  END IF;
  IF has_function_privilege('authenticated','public.get_exam_prep_ai_source_cards_service_v1(text,text,text,text,integer)','EXECUTE')
     OR has_function_privilege('authenticated','public.record_exam_prep_ai_audit_service_v1(uuid,uuid,text,text,text,text,jsonb,text,text,text,text,uuid,text,uuid[],text[],text,text,integer,integer,integer,numeric,text,text[],text)','EXECUTE') THEN
    RAISE EXCEPTION 'P2-74 browser role can execute service-only AI helper';
  END IF;
  IF has_function_privilege('authenticated','private.exam_prep_has_active_protected_assessment_v1(uuid)','EXECUTE')
    ...
    RAISE EXCEPTION 'P2-74 browser role can execute private active-assessment helper';
  END IF;
END
$$;

-- Baseline learner AI must remain unavailable while global AI/policy are OFF.
...
BEGIN
  ...
  IF coalesce((v->>'allowed')::boolean,true)
     OR v->>'mode'<>'unavailable'
     OR v->>'reason'<>'ai_disabled' THEN
...
-- Enable only inside this rolled-back disposable transaction.
...
SET runtime_status='ready',gate_version='p2-74-isolated-shadow-test',updated_at=now()
WHERE capability_code='ai_assist';
UPDATE private.exam_prep_ai_policy
SET generation_enabled=true,updated_at=now()
WHERE id=1;

-- Guard contract: component, intent, locale, input length, mentor authority, and EN/RU/UZ.
SET LOCAL ROLE authenticated;
DO $$
DECLARE
  v jsonb;
...
BEGIN
  ...
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'invalid_component' THEN
    RAISE EXCEPTION 'P2-74 invalid component not blocked: %',v;
  END IF;
  ...
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'interaction_not_allowed' THEN
    RAISE EXCEPTION 'P2-74 forbidden authority intent not blocked: %',v;
  END IF;
  ...
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'locale_not_allowed' THEN
    RAISE EXCEPTION 'P2-74 invalid locale not blocked: %',v;
  END IF;

  v:=public.get_exam_prep_ai_guard_v1('P1','progress_summary','en',2001);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'input_too_long' THEN
    RAISE EXCEPTION 'P2-74 overlong input not blocked: %',v;
  END IF;

  v:=public.get_exam_prep_ai_guard_v1('P1','mentor_report_draft','en',20);
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'mentor_actor_required' THEN
    RAISE EXCEPTION 'P2-74 learner gained mentor-report authority: %',v;
  END IF;
  ...
  FOREACH v_component IN ARRAY ARRAY['P1','P5']::text[] LOOP
    FOREACH v_locale IN ARRAY ARRAY['en','ru','uz']::text[] LOOP
      ...
      IF coalesce((v->>'allowed')::boolean,false) IS NOT TRUE
         ...
         OR v->>'requested_locale'<>v_locale
         OR v->>'policy_version' IS NULL
         OR v->>'prompt_version' IS NULL
         OR v->>'retrieval_policy_version' IS NULL
         OR v->>'response_schema_version' IS NULL THEN
...
-- Source allowlist: current approved progress/plan cards exist for both components and all locales.
-- A draft/blocked card is injected only inside this transaction and must never be returned.
...
BEGIN
  FOREACH v_component IN ARRAY ARRAY['P1','P5']::text[] LOOP
    FOREACH v_locale IN ARRAY ARRAY['en','ru','uz']::text[] LOOP
      ...
      IF jsonb_array_length(v)<>1 THEN
        RAISE EXCEPTION 'P2-74 progress source allowlist count drift component=% locale=% rows=%',v_component,v_locale,v;
      END IF;
      v_expected_key:=lower(v_component)||':progress_context:'||v_locale||':v1';
      IF v#>>'{0,source_card_key}'<>v_expected_key
         OR v#>>'{0,component_code}'<>v_component
         OR v#>>'{0,locale}'<>v_locale THEN
      ...
      IF lower(v::text) LIKE '%correct_answer%'
         ...
         OR lower(v::text) LIKE '%private_explanation%' THEN
  ...
  IF jsonb_array_length(v)<>0 THEN
    RAISE EXCEPTION 'P2-74 no_source precondition failed: unexpected P1 EN theory card=%',v;
  END IF;

  v:=public.get_exam_prep_ai_source_cards_service_v1('P1','en','progress_context',null,20);
  IF v::text LIKE '%p274:p1:blocked:en:v1%' THEN
    RAISE EXCEPTION 'P2-74 draft/blocked prompt-injection source escaped allowlist';
  END IF;
END
$$;
RESET ROLE;

-- Active protected assessment blackout must be subject-wide: P1 active => both P1 and P5 AI blocked.
DO $$
DECLARE
  v_uid uuid;
  v_ass record;
...
BEGIN
  ...
  SELECT a.id AS assessment_id,a.assessment_version,a.component_code,c.id AS content_version_id,c.program_version_id
  INTO v_ass
  ...
  INSERT INTO private.exam_prep_session_authorizations(
    ...
    v_uid,v_ass.assessment_id,'P1','diagnostic','consumed',now()+interval '1 hour','P2-74 protected-session fixture'
  ...
  INSERT INTO private.exam_prep_sessions(
    ...
    'P1','diagnostic','active','p274-protected-session-0001',1
  );
END
$$;
...
BEGIN
  ...
  IF coalesce((v_p1->>'allowed')::boolean,true) OR v_p1->>'mode'<>'blocked' OR v_p1->>'reason'<>'active_assessment' THEN
    RAISE EXCEPTION 'P2-74 active P1 assessment did not black out P1 AI: %',v_p1;
  END IF;
  IF coalesce((v_p5->>'allowed')::boolean,true) OR v_p5->>'mode'<>'blocked' OR v_p5->>'reason'<>'active_assessment' THEN
    RAISE EXCEPTION 'P2-74 active P1 assessment did not black out cross-component P5 AI: %',v_p5;
  END IF;
END
$$;
RESET ROLE;
...
WHERE user_id=(SELECT user_id FROM p274_people WHERE person_key='learner')
  AND client_idempotency_key='p274-protected-session-0001';

-- Independent AI emergency disable must not disable Core.
...
BEGIN
  ...
  IF coalesce((v->>'allowed')::boolean,true) OR v->>'reason'<>'ai_disabled' THEN
    RAISE EXCEPTION 'P2-74 AI disable did not fail closed independently: %',v;
  END IF;
  SELECT * INTO v_cap FROM public.get_exam_prep_capabilities_v1();
  IF coalesce(v_cap.core_access,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'P2-74 disabling AI also disabled Core capability';
  END IF;
END
$$;
RESET ROLE;
...
-- Daily request budget blocks before any provider would be called.
...
BEGIN
  ...
  IF coalesce((v->>'allowed')::boolean,true)
     OR v->>'mode'<>'fallback'
     OR v->>'reason'<>'daily_budget_exhausted' THEN
...
-- Audit is idempotent and cannot mutate academic truth.
SET LOCAL ROLE service_role;
DO $$
DECLARE
  v_uid uuid;
...
BEGIN
  ...
  v_first:=public.record_exam_prep_ai_audit_service_v1(
    v_req,v_uid,'P1','progress_summary','en','fallback',
    jsonb_build_object('p274_shadow',true),
    'exam_prep_ai_policy_v1','exam_prep_ai_prompt_v1','exam_prep_ai_retrieval_v1','exam_prep_ai_response_v1',
    null,'p274-snapshot','{}'::uuid[],ARRAY['p1:progress_context:en:v1'],null,null,5,0,0,0,
    'fake_provider_only',ARRAY['p274_shadow'],'p274-output-hash'
  );
  ...
  IF v_first IS NOT TRUE OR v_second IS NOT FALSE THEN
    RAISE EXCEPTION 'P2-74 audit idempotency failed first=% second=%',v_first,v_second;
  END IF;
END
$$;
RESET ROLE;
...
BEGIN
  ...
  IF row_to_json(a)::text<>row_to_json(b)::text THEN
    RAISE EXCEPTION 'P2-74 AI guard/source/audit path mutated academic state before=% after=%',row_to_json(b),row_to_json(a);
  END IF;
END
$$;
