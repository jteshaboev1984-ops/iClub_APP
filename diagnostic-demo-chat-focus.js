...
function patchClientGuard(){
 ...
 const original=api.guard.bind(api);
 ...
 api.guard=question=>{
  const decision=original(question);if(!decision.blocked)return decision;
 ...
 if(data){
  ...
  const tech=read(TECH_KEY,{});write(TECH_KEY,{...tech,guard_tests:{pass,cases:Object.fromEntries(Object.entries(results).map(([key,value])=>[key,{blocked:value.blocked,reason:value.reason}]))}});
 }
}
