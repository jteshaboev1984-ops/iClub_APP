...
const SOLUTION_INTENT=['solve','calculate','answer','which option','correct option','is my answer','am i right','eliminate','remove two','check my logic','give the letter','determine','реши','посчитай','ответ','какой вариант','правильный вариант','правильно ли','проверь мою логику','убери два','исключи варианты','скажи букву','определи','yech','hisobla','javob','qaysi variant','to‘g‘ri variant','togri variant','men to‘g‘ri','mantiqimni tekshir','ikkita variantni olib tashla','harfni ayt','aniqla'];
const INJECTION_INTENT=['ignore restrictions','ignore previous instructions','pretend the tour is over','system prompt','developer message','игнорируй ограничения','игнорируй предыдущие инструкции','представь что тур закончился','системный промпт','cheklovlarni e’tiborsiz qoldir','oldingi ko‘rsatmalarni e’tiborsiz qoldir','tur tugagan deb tasavvur qil'];
...
function evaluate(question){
 const text=normalize(question);const solutionIntent=phraseHit(text,SOLUTION_INTENT),injectionIntent=phraseHit(text,INJECTION_INTENT);const suspiciousIntent=Boolean(solutionIntent||injectionIntent);let best=null;
 DATA.questions.forEach(item=>{
  ...
  const numericIntent=item.uniqueNumbers.length>=2&&suspiciousIntent&&numbers.length>=Math.min(3,item.uniqueNumbers.length);
  const paraphrase=(numbers.length>=1&&sim>=.38&&terms.length>=1)||(suspiciousIntent&&sim>=.52&&terms.length>=2);
  const optionPattern=options.length>=2&&terms.length>=1;
  const activeTopic=terms.length>=1;
  const score=(exact?100:0)+(numericIntent?45:0)+sim*30+numbers.length*7+terms.length*5+options.length*3;
  const row={item,exact,sim,numbers,terms,options,fingerprint,numericIntent,paraphrase,optionPattern,activeTopic,score};if(!best||row.score>best.score)best=row;
 });
 ...
 let reason='allowed_general';if(blocked)reason=best?.exact?'exact_stem':best?.fingerprint?'numeric_fingerprint':best?.numericIntent?'numeric_intent_fingerprint':best?.optionPattern?'option_pattern':best?.paraphrase?'paraphrase_match':injectionIntent?'prompt_injection':'solution_intent';else if(isActive())reason='theory_only';
...
function recordDecision(decision,layer='client'){const tech=read(sessionStorage,TECH_KEY,{});write(sessionStorage,TECH_KEY,{...tech,guard:{active_tour:isActive(),decision:decision.blocked?'blocked':'allowed',reason:decision.reason,matched_question_id:decision.matchedQuestionId,signals:decision.signals,layer,version:decision.version,model_called:false,quota_charged:false}})}
...
function handleBlocked(question,decision){const card=blockedCard();const chat=read(localStorage,CHAT_KEY,{messages:[],draft:''});chat.messages=Array.isArray(chat.messages)?chat.messages:[];chat.messages.push({id:uid('u'),role:'user',text:String(question).trim(),lang:language(),createdAt:new Date().toISOString()});chat.messages.push({id:uid('a'),role:'assistant',type:'verified',cardId:card.id,variant:'full',guardMode:'blocked',createdAt:new Date().toISOString()});chat.draft='';chat.lastMode='blocked';chat.lastSourceIds=[DATA.version];write(localStorage,CHAT_KEY,chat);recordDecision(decision,'client');G5.openChat({origin:'courses-subject-hub'});scheduleDecorate(20);scrollLatest()}

function ensureMenuButton(){const actions=q('.demo-menu-actions');if(!actions)return;let button=$('demo-active-tour-button');if(!button){button=document.createElement('button');button.type='button';button.className='btn';button.id='demo-active-tour-button';button.dataset.demoMenuAction='active-tour';const learning=$('demo-normal-learning');learning?.insertAdjacentElement('afterend',button)}button.textContent=isActive()?t().menuOn:t().menu}
...
function ensureWarning(){const screen=$('courses-ai-chat');if(!screen)return;let warning=$('demo-active-tour-warning');if(!warning){warning=document.createElement('div');warning.id='demo-active-tour-warning';warning.className='demo-active-tour-warning';const tabs=$('demo-ai-chat-tabs');tabs?.insertAdjacentElement('afterend',warning)}warning.hidden=!isActive();warning.replaceChildren();if(isActive()){const title=document.createElement('b');title.textContent=t().warningTitle;const text=document.createElement('span');text.textContent=t().warning;warning.append(title,text)}const input=$('demo-ai-input');if(input&&isActive())input.placeholder=t().warningTitle}
function ensureGuardButtons(){const tools=q('.demo-ai-composer-tools');if(!tools)return;tools.querySelectorAll('[data-gate7-fill]').forEach(button=>button.remove());if(!isActive())return;[[t().task,'exact'],[t().paraphrase,'paraphrase'],[t().theory,'theory']].forEach(([label,key])=>{const button=document.createElement('button');button.type='button';button.className='demo-ai-fill-btn demo-ai-gate7-chip';button.dataset.gate7Fill=key;button.textContent=label;tools.appendChild(button)})}
function decorateBlocked(){q('#demo-ai-chat-body')?.querySelectorAll('.demo-ai-message.is-assistant').forEach(message=>{const source=message.querySelector('.demo-ai-source span')?.textContent||'';if(!source.includes(t().source)&&!message.querySelector('.demo-ai-answer-section p')?.textContent.includes(t().blockedShort))return;message.classList.add('is-active-tour-blocked');message.querySelector('.demo-ai-actions')?.remove();const badge=message.querySelector('.demo-ai-verified span');if(badge)badge.textContent=t().blocked})}
