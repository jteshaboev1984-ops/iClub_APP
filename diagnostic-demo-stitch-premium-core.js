...
function practiceReadable(){
 const screen=$('courses-practice-start');if(!screen||screen.hidden)return true;
 const nodes=[$('practice-subject-title'),$('practice-stage-meta'),$('practice-best-result-label'),$('practice-best-time-label'),$('practice-best-score'),$('practice-best-time')].filter(Boolean);
 return nodes.length>=6&&nodes.every(node=>{const style=getComputedStyle(node);const rgb=(style.color.match(/\d+(?:\.\d+)?/g)||[]).slice(0,3).map(Number);const light=rgb.length===3?(rgb[0]+rgb[1]+rgb[2])/3:255;return node.textContent.trim().length>0&&style.visibility!=='hidden'&&style.display!=='none'&&Number(style.opacity||1)>.4&&light<205});
}
function audit(){const visibleStudent=qa('.demo-student-context').some(node=>{const style=getComputedStyle(node);return !node.hidden&&style.display!=='none'&&style.visibility!=='hidden'});const overflow=Math.max(0,Math.round(document.documentElement.scrollWidth-document.documentElement.clientWidth));const actionRows=qa('.demo-ai-actions');const actionsGrid=actionRows.every(row=>getComputedStyle(row).display==='grid');const practiceContrast=practiceReadable();const report={version:'stitch-premium-v1.1',checkedAt:new Date().toISOString(),visibleStudent,overflowPx:overflow,actionsGrid,practiceContrast,iconState:'temporary-ai-monogram-awaiting-final-svg',plan:plan(),language:language(),pass:!visibleStudent&&overflow<=2&&actionsGrid&&practiceContrast};const tech=read(sessionStorage,TECH_KEY,{});write(sessionStorage,TECH_KEY,{...tech,premium_audit:report});return report}
