...
async function collectPrimaryScreens(page, label) {
  ...
  const globalBackState = await page.evaluate(() => {
    const el = document.querySelector('#topbar-back');
    if (!el) return { present: false, visible: false, width: 0, height: 0 };
    const r = el.getBoundingClientRect();
    const cs = getComputedStyle(el);
    return {
      present: true,
  ...
  if (!globalBackState.present || !globalBackState.visible) throw new Error('Global app Back control is not visible in Exam Prep');
  if (globalBackState.width < 44 || globalBackState.height < 44) {
    throw new Error(`Global app Back target is only ${globalBackState.width}x${globalBackState.height}px`);
  }
  report.notes.push({ label, globalBackState });
  ...
  if (await firstDetails.count()) {
    ...
    if (await skill.count()) {
      ...
      const skillCopyState = await page.evaluate(() => {
        ...
        return { text: text.trim().slice(0, 1200), forbidden };
      });
      if (skillCopyState.forbidden.length) {
        throw new Error(`Learner skill detail exposes mixed-language canonical copy: ${skillCopyState.forbidden.join(', ')}`);
      }
      report.notes.push({ label, skillCopyState: { forbidden: skillCopyState.forbidden, sample: skillCopyState.text } });
...
(async () => {
  ...
  const qaName = 'QA Mobile Exam Prep 20260924 Latest';
  ...
  console.log(JSON.stringify({
    status: 'MOBILE_QA_LATEST_PRODUCTION_CAPTURED',
