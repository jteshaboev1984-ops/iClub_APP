...
## 7. Functional acceptance boundary and final design handoff (2026-09-17)

The isolated SQL matrices exercise real PostgreSQL academic provenance, retest pass/fail, P1/P5 separation and week rollover, while browser suites exercise the actual application renderer/RPC adapter/session-completion code against deterministic synthetic responses. They are **separate test boundaries**; no claim is made that the browser has completed a live Supabase/PostgreSQL transaction. At tested commit `4f826a72797229a3ac81885667d122ac269cd746`, all 21 PR-triggered workflows finished successfully, including the authorized browser journey. This is sufficient to proceed to the **visual design review**, not permission to release.

To avoid repeatedly adding broad test suites after each successful run, remaining work is focused: final visual refinement from `docs/exam-prep-progress-ux-v1-stitch-handoff.md`, one targeted check of stale planner/read behavior if code changes there, then fresh production read-only baseline and distinct explicit approvals for SQL, merge, deploy and enablement. A combined browser-to-disposable-PostgreSQL transaction remains an additional assurance option rather than a claim of completed coverage; if an observed defect demands it, it becomes a release blocker. Keep the feature off until approval.
